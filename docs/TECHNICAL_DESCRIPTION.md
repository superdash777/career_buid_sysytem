...
│  GET /api/skills-by-category   POST /api/analyze-resume             │
│  GET /api/suggest-skills       POST /api/auth/register|login        │
│  GET /health                   POST /api/auth/refresh|logout         │
│                                GET  /api/auth/me                     │
│                                PATCH /api/auth/onboarding            │
│                                GET|POST /api/analyses · GET /{id}    │
...
│  ┌───────▼───────────────────▼───────────────────────▼───────────┐ │
│  │                    rag_service                                 │ │
│  │  Sentence-Transformers (E5 v2 + MiniLM fallback, см. env)     │ │
...
│  JSON: clean_skills.json, atlas_params_clean.json, skill_synonyms   │
│  SQLite (DB_PATH): users, refresh_tokens, analyses, progress        │
│  bcrypt + JWT (access / refresh) · rate limit на register/login     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| Бэкенд | Python 3.12, FastAPI, Uvicorn, Pydantic v2, bcrypt, PyJWT, SQLite (`db.py`) |
| NLP | pymorphy3 (лемматизация RU), NLTK Snowball (стемминг EN), Sentence-Transformers |
| Эмбеддинги | intfloat/multilingual-e5-large-instruct (1024, skills_v2) + paraphrase-multilingual-MiniLM-L12-v2 (fallback) |
| Векторная БД | Qdrant Cloud (REST API, косинусная метрика) |
| Генеративный ИИ | OpenAI GPT-4o (JSON mode для extraction/normalization/level, temperature=0.0; plan generation temperature=0.3) |
...
Объём набора в репозитории см. README (`clean_skills.json`). Каждая запись:
...
| `GET` | `/api/skills-by-category?profession=...` | Навыки по категориям |
...
| `POST` | `/api/auth/logout` | Выход (отзыв refresh) |
| `GET` | `/api/auth/me` | Текущий пользователь (Bearer) |
| `PATCH` | `/api/auth/onboarding` | Онбординг (Bearer) |
| `GET` | `/api/analyses` | Список анализов (Bearer) |
| `POST` | `/api/analyses` | Сохранить анализ (Bearer) |
| `GET` | `/api/analyses/{analysis_id}` | Детали анализа (Bearer) |
| `GET` | `/api/share/{analysis_id}` | Публичный просмотр результата |
| `GET` | `/api/progress` | Прогресс по навыкам (Bearer) |
| `PATCH` | `/api/progress` | Обновить статус навыка (Bearer) |
...
```
PublicLanding / Auth → Onboarding (опц.) → Dashboard
     → GoalSetup → Skills → Confirmation → Result
                                                  │
                                          ┌───────▼────────┐
                                          │ Выбор gap-ов   │
                                          │ (чекбоксы)     │
...
| `DB_PATH` | `<корень>/data/app.db` | SQLite: пользователи, анализы, прогресс |
| `JWT_SECRET` | `change-me-in-production` | Секрет подписи JWT |
| `JWT_ACCESS_TOKEN_TTL_MINUTES` / `JWT_REFRESH_TOKEN_TTL_MINUTES` | 4320 / 43200 | Сроки токенов |
| `AUTH_LOGIN_RATE_LIMIT` / `AUTH_REGISTER_RATE_LIMIT` | 10 | Лимиты попыток в окне `AUTH_RATE_LIMIT_WINDOW_SEC` (60 с) |
| `PLAN_CONTEXT_MAX_CHARS` | 12000 | Лимит символов контекста для LLM при генерации плана |
