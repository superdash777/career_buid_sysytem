...
## 9. Метрики качества обработки данных (план развития)
...
### 9.4. Метрики gap-анализа

| Метрика | Определение | Целевое значение |
|---|---|---|
| **match%** | Доля навыков/параметров, где уровень пользователя ≥ требуемого | Распределение: 10–70% для типичных кейсов |
| **Gap coverage** | Доля gap-навыков, для которых в JSON есть описание уровня И задачи на развитие | ≥ 70% |
| **Param delta correctness** | Все atlas-параметры показывают корректную delta при одношаговом переходе (= 1) | 100% |

### 9.5. Метрики RAG-контекста для LLM
...
### 9.6. Метрики генерации плана (AI)

| Метрика | Определение | Целевое значение |
|---|---|---|
| **Completeness** | `навыков в ответе / выбранных навыков × 100%` — все ли выбранные навыки получили рекомендации | 100% |
| **JSON validity** | Доля ответов, которые парсятся как валидный JSON с первой попытки | ≥ 90% |
...
### 9.8. План реализации мониторинга
