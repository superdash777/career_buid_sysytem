...
| `DB_PATH` | **Рекомендуется для продакшена** | Путь к файлу SQLite на томе. **Не монтируйте том на `/app/data`** — там лежат `clean_skills.json` и другие файлы из образа; том перекроет каталог и приложение упадёт. Варианты: том на **`/app/var`** и `DB_PATH=/app/var/app.db`, или том на **`/data`** и `DB_PATH=/data/app.db`. Иначе при деплое база пустая и возможен 401 `USER_NOT_FOUND` |
