...
| `DB_PATH` | **Рекомендуется для продакшена** | Абсолютный путь к файлу SQLite на **постоянном томе** (Railway: добавьте Volume и укажите, например, `/data/app.db`). Иначе при каждом деплое база пустая, а старые JWT в браузере дают 401 `USER_NOT_FOUND` |
